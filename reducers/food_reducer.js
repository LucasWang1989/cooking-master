...
function change_loading_data(state, data) {
    return state.set("loading",data.loading)
        .set("food_list_data",data.food_list_data)
        .set("rn",data.rn);
}
...
function food_refresh_step(state,data){
    ...
    if (food_select_item){
        return state.set("food_step_refreshing",data.food_step_refreshing)
            .set("food_select_item",data.food_select_item)
...
function food_list_clear(state,data) {
    ...
    return state.set("food_list_data",Immutable.List())
        .set("rn",0)
        .set("loading",0)
        .set("isRefreshing",false)
}
function clear_food_step(state,data) {
    return state.set("food_select_item", Immutable.List())
        .set("isLike", false)
        .set("food_step_refreshing", false)
}
const tables = {
